...
const tabs: {
	name: "fetch.ts // generic" | "fetch.ts // zod";
	code: string;
}[] = [
	{
		name: "fetch.ts // generic",
		code: `const { data, error } = await betterFetch<{
    userId: string;
    id: number;
    title: string;
    completed: boolean;
	...
	{
		name: "fetch.ts // zod",
		code: `const { data: todos, error: todoError } = await betterFetch("https://jsonplaceholder.typicodei.com/todos/1", {
    output: z.object({
        userId: z.string(),
        id: z.number(),
        title: z.string(),
						...
						<div className="relative">
							<div className="flex flex-col items-start gap-2">
								<div className="relative flex items-center gap-2 mt-2">
									<Cover>
										<p className="relative inline tracking-tight 2xl opacity-90 md:text-3xl lg:text-5xl dark:text-white">
											Better Fetch.
										</p>
									</Cover>
								</div>
							...
							<p className="mt-3 tracking-tight text-zinc-800 dark:text-zinc-300 md:text-2xl">
								Advanced fetch wrapper for typescript with zod schema
								validations, pre-defined routes, callbacks, plugins and more.
							</p>
							{
								<>
									<div className="flex flex-col gap-4 mt-8 font-sans w-fit md:flex-row md:justify-center lg:justify-start">
										<Link
											href="/docs"
		...
		<AnimatePresence initial={false}>
			<MotionConfig transition={{ duration: 0.5, type: "spring", bounce: 0 }}>
				<motion.div
					...
					className="relative overflow-hidden rounded-sm from-stone-100 to-stone-200 dark:from-stone-950/70 dark:to-stone-950/90 bg-gradient-to-tr ring-1 ring-white/10 backdrop-blur-lg"
				>
					<div ref={ref}>
